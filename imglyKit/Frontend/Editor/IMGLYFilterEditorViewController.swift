...
public class IMGLYFilterEditorViewController: IMGLYSubEditorViewController {
    ...
    public private(set) lazy var filterIntensitySlider: UISlider = {
        ...
        slider.addTarget(self, action: #selector(sliderTouchedUpInside), forControlEvents: .TouchUpInside)
    ...
    @objc private func changeIntensity(sender: UISlider?) {
        if changeTimer == nil {
            changeTimer = NSTimer.scheduledTimerWithTimeInterval(updateInterval, target: self, selector: #selector(update), userInfo: nil, repeats: false)
        }
    }
